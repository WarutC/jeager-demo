...
const sdk = new NodeSDK({
  traceExporter: new OTLPTraceExporter({
    url: process.env.TRACING_AGENT_URL ?? 'http://localhost:4317', // (e.g. Jaeger -> http://localhost:4317)
  }),
  instrumentations: [
  ...
  serviceName: process.env.TRACING_SERVICE_NAME ?? 'service-a', // (e.g. 'my-service')  
});
